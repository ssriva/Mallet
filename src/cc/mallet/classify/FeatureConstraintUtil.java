...
public class FeatureConstraintUtil {
	...
	public static HashMap<Integer, double[]> readConstraintsFromHashMap( HashMap<String, HashMap<String, Double>> constraintsHashMap, InstanceList data) {

		HashMap<Integer,double[]> constraints = new HashMap<Integer,double[]>();
		
		for(String featureName:constraintsHashMap.keySet()) {
			int featureIndex = data.getDataAlphabet().lookupIndex(featureName,false);
			HashMap<String, Double> p = constraintsHashMap.get(featureName);
			
			assert( p.size() == data.getTargetAlphabet().size()) : p.size() + " " + data.getTargetAlphabet().size();
			
			double[] probs = new double[p.size()];
			for(String labelName:p.keySet()) {
				int labelIndex = data.getTargetAlphabet().lookupIndex(labelName, false);
				assert(labelIndex != -1) : "Label " + labelName + " not found";
				probs[labelIndex] = p.get(labelName);
			}
			constraints.put(featureIndex, probs);
		}
		return constraints;
